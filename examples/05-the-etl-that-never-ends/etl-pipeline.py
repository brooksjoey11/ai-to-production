...
def process_sales_data(input_file, output_file):
    """
    Process sales data from a CSV file and generate a summary by category.

    Steps:
    1. Read the CSV file.
    2. Validate required columns: 'quantity', 'price', 'category'.
    3. Convert 'quantity' and 'price' to numeric, coercing errors to NaN.
    4. Drop rows with any missing values.
    5. Compute total sales per row: quantity * price.
    6. Aggregate by category: sum of total, count of records.
    7. Save the aggregated result to output_file.
    8. Print the number of records processed (after cleaning).
    """
    try:
    ...
    required_cols = {'quantity', 'price', 'category'}
    missing = required_cols - set(df.columns)
    if missing:
        print(f"Error: Missing required columns: {', '.join(missing)}")
        return

    # Convert quantity and price to numeric, setting invalid entries to NaN
    df['quantity'] = pd.to_numeric(df['quantity'], errors='coerce')
    df['price'] = pd.to_numeric(df['price'], errors='coerce')

    # Drop rows with any missing values (including those from conversion)
    initial_count = len(df)
    df = df.dropna(subset=['quantity', 'price', 'category'])
    dropped = initial_count - len(df)
    if dropped > 0:
        print(f"Warning: Dropped {dropped} rows due to missing or invalid data.")
    ...
    # Aggregate by category
    results = df.groupby('category', as_index=False).agg({
        'total': 'sum',
        'quantity': 'count'   # count of records in category
    })

    # Save results
    try:
        results.to_csv(output_file, index=False)
        print(f"Processed {len(df)} records. Summary saved to '{output_file}'.")
    except Exception as e:
        print(f"Error writing output file '{output_file}': {e}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate sales summary by category from a CSV file.")
    parser.add_argument('input', nargs='?', default='sales_2024.csv',
                        help="Input CSV file (default: sales_2024.csv)")
    parser.add_argument('output', nargs='?', default='sales_summary.csv',
                        help="Output CSV file (default: sales_summary.csv)")
    args = parser.parse_args()

    process_sales_data(args.input, args.output)
>>>>>>> 78e8073 (Sync local changes)
